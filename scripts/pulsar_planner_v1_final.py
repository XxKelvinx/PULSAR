...
def main():
    ...
    stolen = set()
    frames_since_peak = 999
    
    # 1. Post-masking Steal & Recovery (Forward Pass):
    # Rip the bits away immediately AFTER the peak. The ear is "deaf" temporarily.
    # After ~40-60ms, smoothly recover back to the smart tonality logic!
    for i in range(1, total_frames):
        ...
        if frames_since_peak <= 2 and (envelope[i-1] > 1.0 or i-1 in stolen):
            # Deep steal phase (first ~40ms)
            candidate = max(envelope[i-1] * args.post_mask_decay, 0.40)
            if candidate < envelope[i]:
                envelope[i] = candidate
                stolen.add(i)
        elif i-1 in stolen:
            # Recovery phase: climb back up to raw_scales (the "other tactic")
            candidate = envelope[i-1] * 1.5  # 50% recovery per frame
            if candidate < envelope[i]:
                envelope[i] = candidate
                stolen.add(i)
    ...
    # Clip top to max_scale, but let Steal valleys go down to 0.40 (absolute floor)
    # Dropping below 40% (e.g. 10%) collapses the Opus stereo image and causes severe noise!
    bounded = np.clip(envelope, 0.20, args.max_scale)
